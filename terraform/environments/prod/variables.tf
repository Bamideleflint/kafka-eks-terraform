...
variable "cluster_name" {
  ...
  type        = string
  default     = "kafka-eks"  // Changed to avoid conflict
}
...
variable "github_repo" {
  ...
  type        = string
  default     = "Bamideleflint/kafka-eks-terraform"

  validation {
    condition     = can(regex("^[a-zA-Z0-9-_]+/[a-zA-Z0-9-_]+$", var.github_repo))
    error_message = "github_repo must be in format 'username/repository' (e.g., 'Bamideleflint/kafka-eks-terraform')."
  }
}
